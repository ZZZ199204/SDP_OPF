...
Yline_G=xlsread('14bus_G.xlsx','Modified', 'B2:O15');
...
Y_line=zeros(28,(28*c));
for i=1:c
    for j=1:28
        for f=1:28
            Y_line(j,28*(i-1)+f)=Y_line_net(j,f,i);
        end
    end
end
...
subject to
 ...
 Y_line*kron(lambda_lm,eye(28))+M_net*kron((mu_k_M-mu_k_m),eye(28))+Y_react*kron((lambda_k_M-lambda_k_m),eye(28))+Y_act*kron((lambdak_M-lambdak_m),eye(28))+(c11+2*sqrt(c12)*r_1_1)*Y_n_a(:,:,1)+(c21+2*sqrt(c22)*r_2_1)*Y_n_a(:,:,2)>=0:A;
cvx_end
...
disp(eig(Y_line*kron(lambda_lm,eye(28))+M_net*kron((mu_k_M-mu_k_m),eye(28))+Y_react*kron((lambda_k_M-lambda_k_m),eye(28))+Y_act*kron((lambdak_M-lambdak_m),eye(28))+(c11+2*sqrt(c12)*r_1_1)*Y_n_a(:,:,1)+(c21+2*sqrt(c22)*r_2_1)*Y_n_a(:,:,2)));
fprintf('\nThe following is the minimum eigenvalue of the network matrix\n');
disp(min(eig(Y_line*kron(lambda_lm,eye(28))+M_net*kron((mu_k_M-mu_k_m),eye(28))+Y_react*kron((lambda_k_M-lambda_k_m),eye(28))+Y_act*kron((lambdak_M-lambdak_m),eye(28))+(c11+2*sqrt(c12)*r_1_1)*Y_n_a(:,:,1)+(c21+2*sqrt(c22)*r_2_1)*Y_n_a(:,:,2))));
fprintf('\nThe following are the eigenvalues of the dual matrix\n');
disp(eig(A));
[V,D]=eig(Y_line*kron(lambda_lm,eye(28))+M_net*kron((mu_k_M-mu_k_m),eye(28))+Y_react*kron((lambda_k_M-lambda_k_m),eye(28))+Y_act*kron((lambdak_M-lambdak_m),eye(28))+(c11+2*sqrt(c12)*r_1_1)*Y_n_a(:,:,1)+(c21+2*sqrt(c22)*r_2_1)*Y_n_a(:,:,2));
